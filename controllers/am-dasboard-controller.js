...
exports.read = async (req, res) => {
    try {
        const get = await config.connectSms.query(`SELECT k.kategori,l.level,a.id, a.finding, a.status AS status1, b.no_wo, c.status AS status2, a.id_area FROM tr_temuan_h a JOIN mst_kategori k ON k.id = a.kategori JOIN mst_level l ON l.id = a.level JOIN mst_order b ON a.id = b.id_temuan JOIN tr_wo_sap c ON b.no_wo=c.order WHERE a.tanggal_temuan BETWEEN date(date_format(CURDATE(),'%Y-01-01')) AND CURRENT_TIMESTAMP
        UNION ALL
